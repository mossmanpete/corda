...
class StartedMockNode private constructor(private val node: TestStartedNode) {
     ...
     * @param responderFlowClass The class of the responder flow.
     * @return An [Observable] which emits responder flows each time one is executed.
     */
    fun <F : FlowLogic<*>> registerResponderFlow(initiatingFlowClass: Class<out FlowLogic<*>>, responderFlowClass: Class<F>): Observable<F> {
        return node.registerInitiatedFlow(initiatingFlowClass, responderFlowClass)
    }
}
